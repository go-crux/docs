...
<AccordionGroup>
  ...
  <Accordion title="RAMPING — below spend or age thresholds">
    The ad has not yet accumulated enough spend or run for enough days to produce a reliable signal. It is ineligible for fatigue assessment and treated as neutral in portfolio calculations.
  </Accordion>
  ...
  <Accordion title="IMPROVING — recovering from a past peak decline">
    The ad's rolling CAC is 20% or more worse than its own peak, but the 3-day CAC trend is negative (CAC is falling). The ad has deteriorated from its best performance but is actively recovering. It is not paused but monitored closely.
  </Accordion>
...
The default thresholds (20% for FATIGUING, 50% for FATIGUED, 20% above blend for LOW_EFFICIENCY, 4-day rolling window) are configurable in the model's `config_params` CTE.
...
The blended rate combines the healthy-ad decay curve with partial credit for ads currently classified as IMPROVING, controlled by the `improving_credit` parameter. At the default of 0, the forecast is anchored entirely on the healthy-ad survival basis, which gives a realistic rather than optimistic projection.
...
- **`avg_viable_lifespan`** — the area under the blended survival curve. Using Little's Law, this is the expected number of days an ad remains viable after launch. It is used to calculate the steady-state launch rate needed to sustain any given portfolio size.
